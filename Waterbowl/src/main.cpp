...
#define WATER_SENSOR 0 // D0 is right port
//#define WATER_SENSOR PIN_WIRE_SCL // SCL is left port

// init variables
int hasWater;
int light;

TFT_eSPI tft;                        // initilize TFT LCD
...
void setup()
{
    ...
    ////////////////////
    // Init LCD
    // Display only static pics
    tft.begin();
    tft.setRotation(3); // rotate between light and water
    ...
    tft.drawString("Water bowl", 50, 15);       //Drawing a text string
    ...
    tft.setTextSize(2);
    tft.drawString("Sufficient water?", 50, 65);
    tft.setTextSize(3);

    //Setting light
    tft.setTextSize(2);
    tft.drawString("Light", 100, 160);
    tft.setTextSize(3);
    tft.drawString("%", 145, 190);
...
void loop()
{
    ...
    spr.setTextSize(3);
    spr.setTextColor(TFT_BLACK);
    spr.drawNumber(light, 0, 0); //Display water value
    spr.pushSprite(100, 190);    //push to lcd
    spr.deleteSprite();          // clear buffer
    ...
    delay(1000); // wait 1 second, adjust to save battery life
}
